...
const HoloEffect = {
  ...
  fragmentShader: /* glsl */ `
		...
		void main() {
			...
			float holoIntensity = 0.2 + 0.1 * snoise(vec3(0.0, 30.0, uTime));
			

			float direction = mod(vUv.y * 1000. + uTime * 0.5, 4.0)<2.0 ? 1.0 : -1.0;
			uv.x += direction * (4. + 8. * lineOffset) / uSize.x;
			vec3 hologram = texture2D(tDiffuse, uv).rgb;

			vec3 background = vec3(0.9, 0.9, 0.9) * .3;
